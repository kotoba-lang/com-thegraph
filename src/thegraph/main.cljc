(ns thegraph.main
  "Kotodama WASM entrypoint for the TheGraph clean-room actor (L5) — Clojure port.
  ...
  facts (`thegraph.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "thegraph")
(def tier "L5")
...
(def entity-specs
  [{:entity "Subgraph"   :plural "subgraphs"   :id-prefix "thegraph_sub"
    :fields [:name :displayName :description :sourceCodeUrl :imageUrl] :required [:name]
    :coerce {}                       :refs {}}
   {:entity "SubgraphDeployment" :plural "subgraphdeployments" :id-prefix "thegraph_sub"
    :fields [:ipfsHash :stakedTokens :signalledTokens :queryFeesAmount :activeSubgraphCount :transferredToL2] :required [:ipfsHash]
    :coerce {:stakedTokens :int :signalledTokens :int :queryFeesAmount :int :activeSubgraphCount :int :transferredToL2 :bool} :refs {}}
   {:entity "Indexer"   :plural "indexers"   :id-prefix "thegraph_ind"
    :fields [:stakedTokens :allocatedTokens :queryFeesCollected :delegatedTokens :isLegacy] :required [:stakedTokens]
    :coerce {:stakedTokens :int :allocatedTokens :int :queryFeesCollected :int :delegatedTokens :int :isLegacy :bool} :refs {}}
   {:entity "Allocation" :plural "allocations" :id-prefix "thegraph_all"
    :fields [:allocatedTokens :createdAtEpoch :closedAtEpoch :queryFeesCollected :status :isLegacy] :required [:allocatedTokens]
    :coerce {:allocatedTokens :int :createdAtEpoch :int :closedAtEpoch :int :queryFeesCollected :int :isLegacy :bool} :refs {}}
   {:entity "PublishedSubgraph" :plural "publishedsubgraphs" :id-prefix "thegraph_pub"
    :fields [:networkCaip2Id :networkSubgraphId] :required []
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"thegraph.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "thegraph-compat" :tier tier :entities entities} 200])
...
(defn start! [] :thegraph-compat/ready)
